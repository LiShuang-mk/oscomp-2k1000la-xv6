...
namespace klib
{
	...
	public:
		...
		PrinterBuffer( char *buf_ptr, int buf_len ) : _buf( buf_ptr ), _buf_len( buf_len )
		{
			_end_ptr += _buf_len;
		};
		...
		void go_back_one()
		{
			if ( _write_header > _buf ) _write_header--;
		}
	...
	public:
		...
		void print_number( PrinterBuffer &buf, ulong num, int base, print_flag flag, char qualfier,
						   int width );
		...
		static void log_out_va( OutputLevel level, const char *f, uint l, const char *info,
								va_list ap );
