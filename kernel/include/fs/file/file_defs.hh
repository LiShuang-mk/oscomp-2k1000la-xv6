...
namespace fs
{
	...
	*/
	...
	struct FileAttrs
	{
		...
		{
			mode_t mode = 0;
			if( u_read ) mode |= S_IRUSR;
			if( u_write ) mode |= S_IWUSR;
			if( u_exec ) mode |= S_IXUSR;
			if( g_read ) mode |= S_IRGRP;
			if( g_write ) mode |= S_IWGRP;
			if( g_exec ) mode |= S_IXGRP;
			if( o_read ) mode |= S_IROTH;
			if( o_write ) mode |= S_IWOTH;
			if( o_exec ) mode |= S_IXOTH;
			return mode;
		}
	};
