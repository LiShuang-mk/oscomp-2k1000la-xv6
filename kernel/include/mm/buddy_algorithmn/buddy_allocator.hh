...
namespace mm
{
	...
	constexpr u64 buddy_node_magic = 0xbd054a0ef9f5a2deUL;			// Tea hash of buddy_node_magic
	constexpr u64 buddy_unused_magic = 0x9edc5824b97d47b4UL; 		// Tea hash of buddy_unused_magic
	...
	private:
		...
		// BuddyManager _buddy_managers[ num_buddy ];

		// record the order of each node in use
		// uint16 _order_of_used_node[ heap_pages_cnt ];
	...
	private:
		...
		BuddyDesc *_belong_area( BuddyNode *node )
		{
			...
			for ( int i = 0; i < _area_desc_len; i++ )
			{
				...
				if ( addr < ( ulong ) bd._head.addr )
					continue;
				if ( addr >= ( ulong ) bd._head.addr + bd._area_size )
					continue;
				return &bd;
			}
			return nullptr;
		...
		int _split_node( int order, BuddyNode &nd )
		{
			...
			if ( nd._area_size != hsai::page_size << order )
				return -2;
			BuddyNode &inlist = *( ( BuddyNode * ) ( ( ulong ) &nd + ( nd._area_size >> 1 ) ) );
		...
		{
			if ( nd._area_size != nd1._area_size )
				return -1;
			ulong to_sz = nd._area_size << 1;
			nd1 = {
