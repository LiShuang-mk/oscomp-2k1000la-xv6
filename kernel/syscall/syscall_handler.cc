...
#define BIND_SYSCALL(sys_name)                                                          \
	...
	uint64 SyscallHandler::invoke_syscaller(uint64 sys_num)
	{
		...
		if ( sys_num != SYS_write )
		{
			if ( _syscall_name[sys_num] != nullptr )
				printf(YELLOW_COLOR_PRINT "syscall %s\n" CLEAR_COLOR_PRINT, _syscall_name[sys_num]);
			else
	...
	uint64 SyscallHandler::_sys_write()
	{
		...
		if ( _arg_fd(0, &fd, &f) < 0 || _arg_addr(1, p) < 0 || _arg_int(2, n) < 0 ) { return -1; }
	...
	uint64 SyscallHandler::_sys_execve()
	{
		...
		eastl::string path;
		if ( _arg_str(0, path, hsai::page_size) < 0 || _arg_addr(1, uargv) < 0 || _arg_addr(2, uenvp) < 0 ) return -1;
	...
	uint64 SyscallHandler::_sys_uname()
	{
		...
		if ( mm::k_vmm.copyout(*pt, sysa, _SYSINFO_sysname, sizeof(_SYSINFO_sysname)) < 0 ) return -1;
		if ( mm::k_vmm.copyout(*pt, noda, _SYSINFO_nodename, sizeof(_SYSINFO_nodename)) < 0 ) return -1;
		if ( mm::k_vmm.copyout(*pt, rlsa, _SYSINFO_release, sizeof(_SYSINFO_release)) < 0 ) return -1;
		if ( mm::k_vmm.copyout(*pt, vsna, _SYSINFO_version, sizeof(_SYSINFO_version)) < 0 ) return -1;
		if ( mm::k_vmm.copyout(*pt, mcha, _SYSINFO_machine, sizeof(_SYSINFO_machine)) < 0 ) return -1;
		if ( mm::k_vmm.copyout(*pt, dmna, _SYSINFO_domainname, sizeof(_SYSINFO_domainname)) < 0 ) return -1;
	...
	uint64 SyscallHandler::_sys_getdents()
	{
		...
		struct linux_dirent
		{
			...
			char		   d_name[128 - sizeof(d_ino) - sizeof(d_off) - sizeof(d_reclen) -
		...
		mm::PageTable *pt = pm::k_pm.get_cur_pcb()->get_pagetable();
		if ( mm::k_vmm.copyout(*pt, buf_addr, &dirent, sizeof(dirent)) < 0 ) return -1;
	...
	uint64 SyscallHandler::_sys_prlimit64()
	{
		...
		pm::Pcb		  *p  = pm::k_pm.get_cur_pcb();
		mm::PageTable *pt = p->get_pagetable();
		if ( new_limit != 0 ) nlim = ( pm::rlimit64 * )hsai::k_mem->to_vir(pt->walk_addr(new_limit));
		if ( old_limit != 0 ) olim = ( pm::rlimit64 * )hsai::k_mem->to_vir(pt->walk_addr(old_limit));
	...
	uint64 SyscallHandler::_sys_sigaction()
	{
		...
		if ( newactaddr != 0 )
		{
			if ( mm::k_vmm.copy_in(*pt, &a_newact, newactaddr, sizeof(pm::ipc::signal::sigaction)) < 0 ) return -1;
		...
		{
			if ( mm::k_vmm.copyout(*pt, oldactaddr, &a_oldact, sizeof(pm::ipc::signal::sigaction)) < 0 ) return -1;
		}
		return ret;
	}
