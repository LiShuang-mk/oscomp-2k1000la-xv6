...
namespace klib
{
	...
	void Printer::print_number( PrinterBuffer &buf, ulong num, int base, print_flag flag, char qualifier, int width )
	{
		char tmp[ 64 ] = { 0 };
		int tmpi = 0;
		char *digits = nullptr;
		...
		if ( flag.sign == 1 )
		{
			switch ( qualifier )
			{
				case 'h': if ( ( i16 ) num < 0 ) { sign = '-'; num = ( u16 ) -num; } break;
				case 'L':
				case 'l': if ( ( i64 ) num < 0 ) { sign = '-'; num = ( u64 ) -num; } break;
				default:  if ( ( i32 ) num < 0 ) { sign = '-'; num = ( u32 ) -num; } break;
			}
			if ( sign != '-' )
			{
				if ( flag.plus == 1 ) sign = '+';
				else if ( flag.space == 1 ) sign = ' ';
			}
		}
		...
		if ( num == 0 ) { tmp[ tmpi ] = '0'; ++tmpi; }
		else for ( ; num != 0; tmpi++ ) tmp[ tmpi ] = digits[ _divide( num, base ) ];
		...
		if ( flag.prefix == 1 )
		{
			if ( base == 2 ) { buf.put( '0' ); buf.put( 'b' ); }
			else if ( base == 8 ) { buf.put( '0' ); }
			else if ( base == 16 ) { buf.put( '0' ); buf.put( 'x' ); }
		}

		// to pad zero-num

		for ( ; num_pad > 0; num_pad--, tmpi++ )
			tmp[ tmpi ] = '0';
