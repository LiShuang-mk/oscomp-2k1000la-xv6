...
namespace mm
{
	...
	void VirtualMemoryManager::init(const char *lock_name)
	{
		...
		for ( pm::Pcb &pcb : pm::k_proc_pool ) { pcb.map_kstack(mm::k_pagetable); }
	}
	...
	uint64 VirtualMemoryManager::vm_alloc(PageTable &pt, uint64 old_sz, uint64 new_sz, bool executable, bool for_user)
	{
		...
		for ( ulong l = 1, ls = hsai::page_size; l != 0; l <<= 1, ls <<= 1 )
		{
			...
			if ( l & npg )
			{
				...
				bool is_ok;
				if ( executable )
					is_ok = map_code_pages(pt, oa + has_alloc, ls, ( ulong )mem, for_user);
				else
					is_ok = map_data_pages(pt, oa + has_alloc, ls, ( ulong )mem, for_user);
	...
	uint64 VirtualMemoryManager::vm_dealloc(PageTable &pt, uint64 old_sz, uint64 new_sz)
	{
		if ( new_sz >= old_sz ) return old_sz;
	...
	uint64 VirtualMemoryManager::allocshm(PageTable &pt, uint64 oldshm, uint64 newshm, uint64 sz,
	...
	void VirtualMemoryManager::vm_unmap(PageTable &pt, uint64 va, uint64 npages, int do_free)
	{
		uint64	  a;
		hsai::Pte pte;

		if ( (va % hsai::page_size) != 0 ) log_panic("vmunmap: not aligned");

		for ( a = va; a < va + npages * hsai::page_size; a += hsai::page_size )
		{
			if ( (pte = pt.walk(a, 0)).is_null() ) log_panic("vmunmap: walk");
			if ( !pte.is_valid() ) log_panic("vmunmap: not mapped");
			if ( pte.is_dir_page() ) log_panic("vmunmap: not a leaf");
			if ( do_free ) { k_pmm.free_pages(( void * )hsai::k_mem->to_vir(pte.to_pa())); }
			pte.clear_data();
		}
	}
	...
	int VirtualMemoryManager::vm_copy(PageTable &old_pt, PageTable &new_pt, uint64 start, uint64 size)
	{
		...
		if ( !hsai::is_page_align(start) ) log_panic("vm copy : va start not page-align");
		if ( !hsai::is_page_align(size) ) log_panic("vm copy : copy size is not page-align");

		va_end = start + size;
		for ( va = start; va < va_end; va += hsai::page_size )
		{
			if ( (pte = old_pt.walk(va, 0)).is_null() ) log_panic("uvmcopy: pte should exist");
			...
			{
				vm_unmap(new_pt, start, (start - va) / hsai::page_size, 1);
				return -1;
			}
	...
	int VirtualMemoryManager::vm_set_super(PageTable &pt, uint64 va, ulong page_cnt)
	{
